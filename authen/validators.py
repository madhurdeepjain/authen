...
class AcademicValidator:
    ...
    def search_crossref(
        ...
        try:
            if doi:
                ...
                data = self._safe_request(self.crossref_base, params=params)
    ...
    def search_pubmed(self, title: str) -> List[str]:
        ...
        try:
            ...
            search_data = self._safe_request(search_url, params=search_params)
            ...
            if (
                ...
                if pmids:
                    results.append(f"PubMed: Found {len(pmids)} potential match(es).")
        except Exception as e:
            logger.error(f"Error in PubMed search: {e}")
        return results

    def search_arxiv(self, title: str) -> List[str]:
        """Search arXiv API for preprints."""
        results = []
        try:
            params = {"search_query": f'ti:"{title}"', "start": 0, "max_results": 3}
            response_text = self._safe_request(
                self.arxiv_base, params=params, expect_json=False
            )
    ...
    def validate_author(
        self, author_name: str, affiliation: Optional[str] = None
    ) -> Tuple[List[str], Optional[Dict[str, Any]]]:
    ...
    def validate_reference_details(
        ...
        # Search Crossref
        crossref_logs, crossref_meta = self.search_crossref(title or "", doi)
        result.logs.extend(crossref_logs)
        result.merge_reference_metadata(crossref_meta)

        # Search OpenAlex
        openalex_logs, openalex_meta = self.search_openalex(title or "", doi)
        result.logs.extend(openalex_logs)
        result.merge_reference_metadata(openalex_meta)

        # Search Semantic Scholar
        semantic_logs = self.search_semantic_scholar(title or "", doi)
        result.logs.extend(semantic_logs)
        ...
        if title:
            ...
            if any(
                ...
                pubmed_logs = self.search_pubmed(title)
            result.logs.extend(pubmed_logs)
            result.logs.extend(self.search_arxiv(title))

        # Validate authors individually
        for author in authors or []:
            ...
            if isinstance(aff_payload, dict):
                ...
                aff = aff_payload or ""
            author_logs, author_meta = self.validate_author(name, aff)
            result.logs.extend(author_logs)
            normalized_name = normalize_author_key(first, last, fallback=name)
            result.merge_author_metadata(normalized_name, author_meta or {})
